...
from rok4_tools.aspyro_utils.agent import work as agent_work
from rok4_tools.aspyro_utils.finisher import work as finisher_work
from rok4_tools.aspyro_utils.master import work as master_work
...
def parse() -> None:
    ...
    parser = argparse.ArgumentParser(
        prog="aspyro",
        description="Tool to generate a raster pyramid from a WMS service",
        epilog="",
    )
    ...
    if args.role != "example" and (args.configuration is None):
        print("aspyro: error: argument --conf is required for all roles except 'example'")
        sys.exit(1)

    if args.role == "agent" and (args.split is None or args.split < 1):
        print(
            "aspyro: error: argument --split is required for the agent role and have to be a positive integer"
        )
        sys.exit(1)
...
def configuration() -> None:
    ...
    Raises:
        ...
        ValidationError: Configuration is not a valid ASPYRO configuration file
    ...
    f = open(os.path.join(os.path.dirname(__file__), "aspyro_utils", "schema.json"))
    schema = json.load(f)
    f.close()
    ...
    if "slab_size" not in config["pyramid"]:
        config["pyramid"]["slab_size"] = [16, 16]
...
def main() -> None:
    ...
    if args.role == "example":
        ...
        f = open(os.path.join(os.path.dirname(__file__), "aspyro_utils/example.json"))
        print(f.read())
        f.close
        sys.exit(0)
